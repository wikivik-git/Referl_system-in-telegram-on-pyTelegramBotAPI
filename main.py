...
def start(message):
        ...
        bot.send_message(message.chat.id, 'Привет, сейчас я тебя зарегестрирую и дам тебе твою реферальную ссылку') 
        ...
        #Проверяем наличие пользователя в базе данных, если его нет, то регестрируем
        if cur.fetchone() is None:
            cur.execute('INSERT INTO Referalka (id, Referal) VALUES (?, ?)', (user_id, refer_id))
            conn.commit()
            cur.close()
            bot.send_message(message.chat.id, 'Поздравляю! Ты успешно зарегестриван')
        conn.commit()
        cur.close()
...
#Обрабатываем команду number_of_referrals, с помощью которой можно узнать сколько человек перешло по реферальной ссылке 
